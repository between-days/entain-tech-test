...
Tests are mostly written by ai because I had to spam them out, races only has the one unit test for enrichRace because it's the only clean non-mock-requiring new code. Basic arbitrary happy path integration tests are used for the new features requiring spec-down-to-db changes on tasks 1-4. Task 5 is the only integration test not using the seed simply because it's most economical to test only the new things, sports is entirely new so should be a little more stringent on it's 'default' features. The chosen tests/rigor of each is meant to demonstrate the importance of tests in each case.
